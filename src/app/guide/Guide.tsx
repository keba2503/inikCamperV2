...
const GuideAccordion: React.FC = () => {
    ...
    const [translatedGuides, setTranslatedGuides] = useState<Guide[]>([]);
    ...
    useEffect(() => {
        const translateGuides = async () => {
            try {
                ...
                setTranslatedGuides(translations);
            } catch (error) {
                console.error("Error translating guides:", error);
            }
        };
    ...
    return (
        <div className="w-full max-w-6xl mx-auto pt-12 p-6">
            {translatedGuides.map((guide, index) => (
