...
const Page = () => {
    ...
    const [translatedFounders, setTranslatedFounders] = useState<People[]>([]);
    ...
    useEffect(() => {
        ...
        const translateFounders = async () => {
            try {
                const translations = await Promise.all(
                    FOUNDER_DEMO.map(async (founder) => ({
                        ...founder,
                        name: await translateText(founder.name, language),
                        job: await translateText(founder.job || "Team Member", language),
                    }))
                );
                setTranslatedFounders(translations);
            } catch (error) {
                console.error("Error translating founders:", error);
            }
        };
                        ...
                        <div className="grid sm:grid-cols-2 gap-x-5 gap-y-8 lg:grid-cols-4 xl:gap-x-8 pt-10">
                            {translatedFounders.map((item) => (
                                <div key={item.id} className="max-w-xs mx-auto">
                                    ...
                                    <span
                                        className="block text-sm text-neutral-500 sm:text-base dark:text-neutral-400 text-center">
                                        {item.job}
                                    </span>
                                </div>
                            ))}
